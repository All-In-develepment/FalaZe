...
const prepareLocation = (msg: WbotMessage): WbotMessage => {
  ...
  msg.body += `|${
    msg.location.description ? msg.location.description : msg.location.latitude
  },${msg.location.longitude}`;
...
const buildLastMessage = (msg: WbotMessage) => {
  if (msg.type === "location") {
    if (msg.location.description) {
      return `Localization - ${msg.location.description.split("\\n")[0]}`;
    }
    return "Localization";
  }
  return msg.body;
...
const verifyMessage = async (
  ...
  const lastMessage = buildLastMessage(msg);

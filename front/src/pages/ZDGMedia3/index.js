...
async function FALAZESender(number, url, iD, token) {
	...
	const handleChange = (event) => {
	  ...
	  const handleSubmit = async (event) => {
		...
		for (const user of usersTextArea){
			...
			if (numberDDI !== "55") {
				FALAZESender(user, inputs.url, inputs.id, token);
				await timer(rndInt * 1000)
				...
				await timer(rndInt * 1000)
				FALAZESender(numberDDI.toString() + numberDDD.toString() + "9" + numberUser.toString(), inputs.url, inputs.id, token);
				alert('Mensagem enviada para o número: ' + numberDDI.toString() + numberDDD.toString() + "9" + numberUser.toString());
			}
			else if (numberDDI === "55" && parseInt(numberDDD) > 30) {
				const numberUser = user.substr(-8,8);
				await timer(rndInt * 1000)
				FALAZESender(numberDDI.toString() + numberDDD.toString() + numberUser.toString(), inputs.url, inputs.id, token);
